...
public class SwipeHelper implements View.OnTouchListener {
    ...
    private void swipeViewToLeft(int duration) {
        if(this.mListenForTouchEvents) {
            ...
            this.mObservedView.animate().x((float)(-this.mSwipeStack.getWidth()) + this.mObservedView.getX()).rotation(-this.mRotateDegrees).alpha(0.0F).setDuration((long)duration).setListener(new AnimationUtils.AnimationEndListener() {
    ...
    private void swipeViewToBottom(int duration) {
        if(this.mListenForTouchEvents) {
            this.mListenForTouchEvents = false;
            this.mObservedView.animate().cancel();
            this.mObservedView.animate().y((float)this.mSwipeStack.getHeight() + this.mObservedView.getY()).rotation(this.mRotateDegrees).alpha(0.0F).setDuration((long)duration).setListener(new AnimationUtils.AnimationEndListener() {
                public void onAnimationEnd(Animator animation) {
